...
// This is cleaner and lets us close the menu at the same time
function handleLogout() {
  userStore.logout()
  navMenu.value = false // Close menu on logout
}
</script>
    ...
    <div class="nav-links-menu">
      <nav class="nav-links">
        ...
        <div v-if="navMenu" class="nav-account-menu">
          <router-link to="/account" @click="navMenu = false"> My account </router-link>
          <router-link v-if="!userStore.isLoggedIn()" to="/login" @click="navMenu = false">
            se connecter
          </router-link>
          <router-link v-else to="/" @click="handleLogout"> se déconnecter </router-link>
        </div>
      </nav>
    </div>
  </div>
